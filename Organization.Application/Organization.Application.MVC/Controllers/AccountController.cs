...
namespace Organization.Application.MVC.Controllers
{
    ...
    public partial class AccountController : BaseController
    {
        ...
        public async virtual Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var user = await CustomUser.FindByNameAsync(model.Email.Trim());
            if(user == null)
            {
        ...
        public virtual async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null && !user.PersonRoles.Any(r => r.RoleId == model.RoleID))
                {
                    ...
                    message.Add($"We found your email address in our system and you already created an account with us as <strong> {existedRoleName} </strong>.<br> To create new role please do the following:<ul><ol>Log in to your account.</ol> <ol>Click on Profile link</ol><ol>Click on Add New Role</ol></ul> Click here to go to login page <a class='btn btn-info' href='{loginUrl}'>LogIn</a> ");
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        string emailBody = String.Format(emailTemplate.Body, callbackUrl);
